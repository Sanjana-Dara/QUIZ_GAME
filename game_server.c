...
int main()
{
	...
	char buf1[100],buf2[100]="\0";char msg[100]="YOUR SWEET NAME PLEASE >>>"; char name[100];
	char names[10][100]; int times[10][6]; char que[5][500]; char ans[5][100]; char answer[100];char ack[100];char win[100];char end[10];
	
	strcpy(que[0],"WHO IS CEO OF GOOGLE ?\nA)TIM COOK\nB)SUNDHAR PICHAI\nC)SATYA NADELLA\nD)JEFF BEZOS\n");
	strcpy(que[1],"EVALUATE THE GIVEN EXPRESSION :2+2*2-2%2 = ?\nA)4\nB)5\nC)2\nD)3\n");
	strcpy(que[2],"NAME THE LEAD MALE ACTOR OF BAAHUBALI?\nA)SATHYARAJ\nB)NASSAR\nC)PRABHAS\nD)RANA\n");
	strcpy(que[3],"WHICH COURSE DOES THIS PROJECT BELONGS TO?\nA)OS\nB)VLSI\nC)DBMS\nD)COMPUTER NETWORKS\n");
	strcpy(que[4],"WHAT IS THE NAME OF GIRLS HOSTEL OF IIITDM?\nA)ASHOKA\nB)ASWATHA\nC)JASMINE\nD)LOTUS\n");
	strcpy(ans[0],"B");
	strcpy(ans[1],"B");
	strcpy(ans[2],"C");
	strcpy(ans[3],"D");
